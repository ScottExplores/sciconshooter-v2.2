...
export class GameEngine {
  ...
  bgScrollY: number = 0; // Background scroll offset
  ...
  constructor(stats: Stats) {
    ...
    loadImage(ASSETS.FOUNDER_ARSHIA);
    loadImage(ASSETS.RSC_TOKEN);
    loadImage(ASSETS.MINI_BOSS_SPRITE);

    // Preload NEW enemy sprites
    loadImage(ASSETS.ENEMY_DRONE_SPRITE);
    loadImage(ASSETS.ENEMY_SWARM_SPRITE);
    loadImage(ASSETS.ENEMY_JOURNAL_SPRITE);
    loadImage(ASSETS.ENEMY_BRICK_SPRITE);
    loadImage(ASSETS.ENEMY_INVADER_SPRITE);

    // Preload NEW boss sprite
    loadImage(ASSETS.BOSS_SPRITE);

    // Preload NEW backgrounds
    loadImage(ASSETS.BG_SPACE);
    loadImage(ASSETS.BG_STARS);
    loadImage(ASSETS.BG_SPACE_PURPLE);
    loadImage(ASSETS.BG_SCIFI);
    loadImage(ASSETS.BG_SCIFI_SKY);
  }
  ...
  draw(ctx: CanvasRenderingContext2D) {
    // Wave-based background selection for level variety
    let bgAsset = ASSETS.BG_SPACE; // Default blue nebula

    // Rotate backgrounds based on wave number
    const waveTheme = this.wave % 5;
    switch (waveTheme) {
      case 1:
        bgAsset = ASSETS.BG_SPACE; // Blue nebula
        break;
      case 2:
        bgAsset = ASSETS.BG_SPACE_PURPLE; // Purple space
        break;
      case 3:
        bgAsset = ASSETS.BG_SCIFI; // Sci-fi environment
        break;
      case 4:
        bgAsset = ASSETS.BG_SCIFI_SKY; // Sci-fi sky
        break;
      case 0:
        bgAsset = ASSETS.BG_SPACE; // Back to blue
        break;
    }


    // Scroll background for movement effect
    this.bgScrollY += 0.5; // Slow scroll speed
    if (this.bgScrollY >= this.height) {
      this.bgScrollY = 0;
    }

    const bgImg = imageCache[bgAsset];
    if (bgImg && bgImg.complete && bgImg.naturalWidth > 0) {
      // Dim the background so it's less distracting
      ctx.save();
      ctx.globalAlpha = 0.4; // 40% opacity - much dimmer

      // Draw scrolling tiled background
      const pattern = ctx.createPattern(bgImg, 'repeat');
      if (pattern) {
        ctx.translate(0, this.bgScrollY);
        ctx.fillStyle = pattern;
        ctx.fillRect(0, -this.height, this.width, this.height * 2);
      }
  ...
  drawMiniBoss(ctx: CanvasRenderingContext2D, e: Entity) {
    ...
    const img = imageCache[ASSETS.MINI_BOSS_SPRITE];
    if (img && img.complete && img.naturalWidth > 0) {
      // Render mini-boss sprite (bottleneck icon is already oriented correctly)
  ...
  drawBoss(ctx: CanvasRenderingContext2D, e: Entity) {
    ...
    // Try to render boss sprite first
    const img = imageCache[ASSETS.BOSS_SPRITE];
    if (img && img.complete && img.naturalWidth > 0) {
      ctx.save();
      if (e.isCharging) {
        ctx.shadowBlur = 50;
  ...
  drawEntity(ctx: CanvasRenderingContext2D, e: Entity) {
    ...
    if (e.type === EntityType.PLAYER) {
      ...
      // DRAW WALL BOSS BLOCKS WITH SPRITE
      const img = imageCache[ASSETS.ENEMY_INVADER_SPRITE];
      if (img && img.complete && img.naturalWidth > 0) {
        ctx.save();
      ...
      // ENEMY RENDERING WITH SPRITES
      let spriteUrl = null;

      if (e.type === EntityType.ENEMY_DRONE) {
        spriteUrl = ASSETS.ENEMY_DRONE_SPRITE;
      } else if (e.type === EntityType.ENEMY_SWARM) {
        spriteUrl = ASSETS.ENEMY_SWARM_SPRITE;
      } else if (e.type === EntityType.ENEMY_JOURNAL) {
        spriteUrl = ASSETS.ENEMY_JOURNAL_SPRITE;
      } else if (e.type === EntityType.ENEMY_BRICK) {
        spriteUrl = ASSETS.ENEMY_BRICK_SPRITE;
      } else if (e.type === EntityType.ENEMY_INVADER) {
        spriteUrl = ASSETS.ENEMY_INVADER_SPRITE;
      }

      if (spriteUrl) {
        const img = imageCache[spriteUrl];
        if (img && img.complete && img.naturalWidth > 0) {
          // Rotate sprite 180 degrees so enemies face downward
          ctx.save();
          ctx.rotate(Math.PI);
          ctx.drawImage(img, -e.width / 2, -e.height / 2, e.width, e.height);
          ctx.restore();
        ...
        // Non-enemy entities - use original rendering
        ctx.fillStyle = e.color;
        if (e.text) {
          ctx.font = '24px Arial';
          ctx.textAlign = 'center';
